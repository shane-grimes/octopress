...
module Octopress
  class JSAssetsManager
    ...
    def initialize
      ...
      @dependencies = @config[:require_js][:dependencies].collect {|item| Dir.glob("#{@js_assets_path}/#{item}") }.flatten.uniq
      @modules = @config[:require_js][:modules].collect {|item| Dir.glob("#{@js_assets_path}/#{item}") }.flatten.uniq

      @template_path = File.expand_path("../../#{@config[:source]}", File.dirname(__FILE__))
